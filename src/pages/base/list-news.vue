    ...
    <div ref="mescroll" class="mescroll">
      <div>
        ...
        <ul id="newsList" class="news-list">
          <li v-for="news in newArr" :key="news.status">
            <p>{{news.status}}</p>
            <p class="new-content">{{news.statusDesc}}</p>
          </li>
        </ul>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    downCallback () {
      ...
      axios.get("http://yapi.demo.qunar.com/mock/50628/?debug=longpong", {
        params: {
          num: 0, //页码
          size: 1 //每页长度
        }
    ...
    upCallback (page) {
      ...
      axios.get("http://yapi.demo.qunar.com/mock/50628/?debug=longpong", {
        ...
        //如果是第一页需手动制空列表
        //把请求到的数据添加到列表
        this.newArr = this.newArr.concat(arr);
        //数据渲染成功后,隐藏下拉刷新的状态
        this.$nextTick(() => {
          this.mescroll.endSuccess(arr.length);
        })
